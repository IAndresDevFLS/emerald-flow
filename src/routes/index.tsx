      ...
      <main id="main" className="pt-16">
        ...
        <section className="mx-auto max-w-[1200px] px-6 py-8 md:py-10" aria-label="Listado de enlaces">
          ...
          <div className="hidden overflow-hidden rounded-2xl border border-border bg-card shadow-card md:block">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] border-collapse text-left">
                ...
                <tbody>{filtered.map((item, index) => <LinkRow key={item.id} item={item} index={index} copyLink={copyLink} duplicateLink={duplicateLink} deleteLink={deleteLink} />)}</tbody>
              </table>
            </div>
          ...
          <div className="grid gap-4 md:hidden">
            {filtered.map((item, index) => <MobileCard key={item.id} item={item} index={index} copyLink={copyLink} duplicateLink={duplicateLink} deleteLink={deleteLink} />)}
...
type RowProps = { item: LinkItem; index: number; copyLink: (item: LinkItem) => void; duplicateLink: (item: LinkItem) => void; deleteLink: (id: number) => void };
